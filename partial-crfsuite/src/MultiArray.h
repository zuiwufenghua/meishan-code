...
public:
	int resize(const vector<unsigned int> &vecDimension, const Ty &default_data) {
		...
		for (; i >= 0; --i) {
			if (vecDimension[i] == 0) {
				cout << "dimension element num is 0" << endl;
				return -1;
			}
...
/*		} catch (const exception &e) {
			cout << "MultiArray::vector::resize( " << arr_size + 1 << " ) exception" << endl;
...
*/
	...
	Ty &getElement(unsigned int pos) {
		if (pos >= end_pos()) {
			cout << "position err: reach end: " << pos << endl;
...
private:
	unsigned int getPosition(const vector<unsigned int> &vecDemension) const 
	{
		...
		{
			cout << "dimension not equal, should be: " << m_vecDimension.size() << " not: " << vecDemension.size() << endl;
			return end_pos();
		}
		unsigned int pos = 0;
		int i = 0;
		for (; i < vecDemension.size(); ++i) {
			...
			{
				cout << "dimension " << i << " value err: " << vecDemension[i] << endl;
				cout << "dimension size is: " << m_vecDimension[i] << endl;
				return end_pos();
			}
