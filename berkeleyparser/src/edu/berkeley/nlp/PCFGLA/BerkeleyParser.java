...
public class BerkeleyParser  {
	...
	public static class Options {
		...
		@Option(name = "-useCandPosNum", usage = "max candidate pos number use.")
		public int nPosNum = 0;
	...
	public static void main(String[] args) {
		...
		CoarseToFineMaxRuleParser accurate_parser = null;
		if (opts.nGrammars != 1){
      ...
      if(!opts.accurate)
      {
    	  accurate_parser = new CoarseToFineMaxRuleProductParser(grammars, lexicons, threshold,-1,opts.viterbi,opts.substates,opts.scores, true, opts.variational, true, true);
    	  accurate_parser.binarization = bin;
      }
      parser.binarization = bin;
			...
			if(!opts.accurate)
			{
				if (opts.kbest==1) accurate_parser = new CoarseToFineMaxRuleParser(grammar, lexicon, threshold,-1,opts.viterbi,opts.substates,opts.scores, true, opts.variational, true, true);
				else accurate_parser = new CoarseToFineNBestParser(grammar, lexicon, opts.kbest,threshold,-1,opts.viterbi,opts.substates,opts.scores, true, opts.variational, false, true);
				accurate_parser.binarization = pData.getBinarization();
				accurate_parser.nPosCands = opts.nPosNum;
			}
			
			parser.nPosCands = opts.nPosNum;
		}
			...
			while((line=inputData.readLine()) != null){
			  ...
			  if (opts.ec_format && line.equals("")) continue;  
				...
				if (opts.goldPOS){
					...
					sentence.add(tmp.get(1).trim());
					//String[] tags = tmp.get(1).split("-");
					//posTags.add(tags[0]);
					posTags.add(tmp.get(9).trim());
				...
				if (opts.nThreads > 1){
					...
					if (opts.kbest > 1){
						...
						if (parsedTrees.size()==0 && accurate_parser != null) {
							parsedTrees = accurate_parser.getKBestConstrainedParses(sentence, posTags, opts.kbest);
						}
						...
						if (parsedTree.getChildren().isEmpty() && accurate_parser != null) {
							System.out.println("start accurate parser for sentence: " + String.format("%d", sentenceNum+1));
							parsedTree = accurate_parser.getBestConstrainedParse(sentence,posTags,null);
						}
						...
						if (opts.goldPOS && parsedTree.getChildren().isEmpty() && accurate_parser != null){ // parse error when using goldPOS, try without
							parsedTree = accurate_parser.getBestConstrainedParse(sentence,null,null);
						}
				...
				sentenceNum++;
				if(sentenceNum%100 == 0)
				{
					long end = System.currentTimeMillis();
					System.out.println(String.format("sen num:%d, time:%d", sentenceNum, (end-start)/1000));
				}
			}
			long end = System.currentTimeMillis();
			System.out.println(String.format("sen num:%d, time:%d", sentenceNum, (end-start)/1000));
