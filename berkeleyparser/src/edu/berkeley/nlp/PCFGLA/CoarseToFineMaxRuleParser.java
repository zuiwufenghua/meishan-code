...
//  	this.numSubStatesArray = gr.numSubStates.clone();
  ...
  public Numberer getCurNumberer()
  {
	  return tagNumberer;
  }
  ...
  void doConstrainedOutsideScores(Grammar grammar, boolean viterbi, boolean logScores) {
	...
	void initializeChart(List<String> sentence, Lexicon lexicon,boolean noSubstates,
		...
		for (String word : sentence) {
			...
			//int goldTag = -1;
			boolean[] goldTags = new boolean[numStates];
			for (int tag=0; tag<numStates; tag++){
				goldTags[tag] = false;
			}
      if (useGoldPOS && posTags!=null) {
    	//goldTag = tagNumberer.number(posTags.get(start));
			String[] tags = posTags.get(start).split("_");
			int candCount = 0;
			for(String curTag : tags)
			{
				int goldTag = tagNumberer.number(curTag);
				if(goldTag >= 0 && goldTag <numStates && !grammarTags[goldTag])
				{
					goldTags[goldTag] = true;
					candCount++;
					if(nPosCands != 0 && candCount >= nPosCands)break;
				}
			}
			if(candCount == 0)
			{
		...
		if (useGoldPOS  && posTags!=null && !goldTags[tag]) continue;
...
//	  	double[] accurateThresholds =  {-8,-12,-12,-11,-12,-12,-14,-14};
        double[] accurateThresholds =  {-20,-24,-24,-24,-24,-24,-26,-26};	  	
