...
using namespace Cnf;
Cnftagger::Cnftagger(const char *tmpl) {
	this->tmpl = tmpl;
	/// sqcolsize
	this->sqcolsize = 3;
	/// unigram feature dic
...
std::string Cnftagger::expand(char *tp, Sequence *s, int current,
		std::string& f) {
	f = "";
	for (char *p = tp; *p != '\0'; p++) {
		if (*p == '%' && *(p + 1) == 'x' && *(p + 2) == '[') {
...
void Cnftagger::storefset(Sequence *sq, std::vector<feature_t>& featureset) {
...
void Cnftagger::output(Sequence *s, std::vector<int>& labels) {
	...
	for (int i = 0; i < row; i++, rit++) {
		for (int j = 0; j < (int) this->sqcolsize; j++) {
			fprintf(stdout,"%s ",s->getToken(i,j).c_str());
		}
		...
		for (int j = 0; j < row; j++) {
			...
			for (; uit != featureset[i].uf.end(); uit++, utit++) {
				...
				if (this->fwit[tmpl] == this->fwit[tmpl + 1]) {
					...
					float a = 0.;
					for (int k = this->fwit[tmpl]; k < this->fwit[tmpl + 1];
							k++) {
						a += *(this->model + k);
					}
...
float Cnftagger::getbcost(int bias, int label, feature_t *featureset) {
...
void Cnftagger::viterbi(Sequence *s, std::vector<int>& lids) {
	if (!this->valid) {
	...
	/// build lattice
				...
				int row = this->labelsize;
				vnode **lattice = (vnode**)malloc(sizeof(vnode*)*col);
				for (int c = 0; c < col; c++)
				{
					lattice[c] = (vnode*)malloc(sizeof(vnode)*row);
				}
