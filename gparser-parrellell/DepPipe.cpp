...
DepPipe::DepPipe(const ParserOptions &_options) : options(_options)
{
  ...
  {
    tp = create_threadpool(options.m_threadnum);
  }

	mut_counter= (sp_thread_mutex_t*) malloc (sizeof(sp_thread_mutex_t));
	sp_thread_mutex_init(mut_counter, NULL);

	mut_iter = (sp_thread_mutex_t*) malloc (sizeof(sp_thread_mutex_t));
	sp_thread_mutex_init (mut_iter, NULL);
    
	cond_iter_over = (sp_thread_cond_t*) malloc (sizeof(sp_thread_cond_t));
	sp_thread_cond_init(cond_iter_over, NULL);
}
...
DepPipe::~DepPipe(void)
{
  ...
  if(options.m_threadnum > 1)
  {
	...
	sp_thread_mutex_destroy (mut_counter);
	free(mut_counter);

	sp_thread_cond_destroy (cond_iter_over);
	free(cond_iter_over);
	
	sp_thread_mutex_destroy(mut_iter);
	free(mut_iter);
}
...
int DepPipe::addLabelFeature(DepInstance *pInstance, int nodeIdx, const string &deprel, bool is_child, bool attR, FeatureVec &fv)
{
  int newFeatureCount = 0;
	...
	const string &guide_arc = pInstance->lemmas[nodeIdx];
...
//*/
	...
	////if (options.m_isUseForm && options.m_isUseGuideLab && options.m_isUseForm_label) 
 // {
	...
	//}
	////if (options.m_isUseGuideArc && options.m_isUseGuideLab && options.m_isUseLemma_label)
 // {
	//	feat = prefix + "2" + guide_arc + " " + postag;
	//	if (options.m_isUse_label_feats_t) newFeatureCount += add(feat, fv);
	//	if (options.m_isUse_label_feats_t_child) newFeatureCount += add(feat + strIsChild, fv);
	//	newFeatureCount += add(feat + dir_child, fv);
	//}
	//if (options.m_isUseForm && options.m_isUseCPostag && options.m_isUseForm_label) 
  {
	...
	//if (options.m_isUseGuideArc && options.m_isUseCPostag && options.m_isUseLemma_label) 
  /*{
		feat = prefix + "4" + guide_arc + " " + cpostag;
		if (options.m_isUse_label_feats_t) newFeatureCount += add(feat, fv);
		if (options.m_isUse_label_feats_t_child) newFeatureCount += add(feat + strIsChild, fv);
		newFeatureCount += add(feat + dir_child, fv);
	}*/
	...
	//if (options.m_isUseForm && options.m_isUseForm_label) 
  {
	...
	//if (options.m_isUseGuideArc && options.m_isUseLemma_label) 
  /*{
		feat = prefix + "2" + guide_arc;
		if (options.m_isUse_label_feats_t) newFeatureCount += add(feat, fv);
		if (options.m_isUse_label_feats_t_child) newFeatureCount += add(feat + strIsChild, fv);
		newFeatureCount += add(feat + dir_child, fv);
	}*/
	//if (options.m_isUseGuideLab) 
  /*{
...
int DepPipe::addLabelFeature_surrounding(const string &prefix, const vector<string> &vecInfo, int nodeIdx, const string &deprel, const string &strIsChild, const string &dir, FeatureVec &fv)
{
  int newFeatureCount = 0;
	...
	//if (options.m_isUse_label_feats_t) 
  newFeatureCount += add(feat, fv);
	//if (options.m_isUse_label_feats_t_child) 
  newFeatureCount += add(feat + strIsChild, fv);
	newFeatureCount += add(feat + dir_child, fv);
...
void DepPipe::fillFeatureVectors(DepInstance *instance, FVS_PROBS* pFvsProbs, const Parameter &params, int start_pos, int end_pos)
{
  ...
  if(options.m_threadnum > 1)
  {
	  sp_thread_mutex_lock(mut_counter);
	  counter_++;
	  if (counter_ == t_ ) 
	  {
		  counter_ = 0;
		  //usleep(1000);			
		  sp_thread_cond_signal (cond_iter_over);
	  }
	  sp_thread_mutex_unlock(mut_counter);
  }
...
int DepPipe::addFeature_bi_mcdonald06( DepInstance *pInstance, const string &deprel, const int head_id, const int child_id, FeatureVec &fv)
{
  ...
  if(options.m_isUseFeats)
  {
    int featNum = pInstance->feats[0].size();
    for(int feat_index = 0; feat_index < featNum; feat_index++)
    {
      ostringstream out;
      out.str("");
		  out << feat_index;
      const string prefix_1 = "feats-" + out.str();

      const string &cfeat_h = pInstance->feats[head_id][feat_index];
	    const string &cfeat_c = pInstance->feats[child_id][feat_index];

      if (arc_info.is_root)
		  {
			  const string prefix = prefix_1 + "-BB-";

				  feat = prefix + "01=" + forms[head_id] + FEAT_SEP + forms[child_id] + FEAT_SEP + cfeat_c; // word-0 word-j pos-j
				  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

				  feat = prefix + "03=" + forms[head_id]+ FEAT_SEP + cfeat_c; // word-0 pos-j
				  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);
		  } // end if (is_root)
		  else
		  {
			  ...
			  feat = prefix + "0=" + forms[head_id] + FEAT_SEP + cfeat_h + FEAT_SEP + forms[child_id] + FEAT_SEP + cfeat_c; // word-i pos-i word-j pos-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  feat = prefix + "1=" + forms[head_id] + FEAT_SEP + forms[child_id] + FEAT_SEP + cfeat_c; // word-i word-j pos-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  feat = prefix + "2=" + cfeat_h + FEAT_SEP + forms[child_id] + FEAT_SEP + cfeat_c; // pos-i word-j pos-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  feat = prefix + "3=" + forms[head_id] + FEAT_SEP + cfeat_h + FEAT_SEP + forms[child_id]; // word-i pos-i word-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  feat = prefix + "4=" + forms[head_id] + FEAT_SEP + cfeat_h + FEAT_SEP + cfeat_c; // word-i pos-i pos-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  // not in mcdonald-phd-thesis06
			  feat = prefix + "5=" + forms[head_id] + FEAT_SEP + cfeat_c; // word-i pos-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  feat = prefix + "6=" + cfeat_h + FEAT_SEP + forms[child_id]; // pos-i word-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);

			  feat = prefix + "8=" + cfeat_h + FEAT_SEP + cfeat_c; // pos-i pos-j
			  newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir_dist, fv);
		  }
    }
  }
...
int DepPipe::addFeature_bet_mcdonald06( DepInstance *pInstance, const string &deprel, const int head_id, const int child_id, FeatureVec &fv)
{
  ...
  int newFeatureCount = 0;
	...
	if (arc_info.is_root)
	{
		const string &pos_root = pInstance->cpostags[0];
		if (options.m_isUseCPostag) 
    {
...
int DepPipe::addFeature_sur_mcdonald06( DepInstance *pInstance, const string &deprel, const int head_id, const int child_id, FeatureVec &fv)
{
  ...
  if(options.m_isUseFeats)
  {
    ...
    for(int feat_index = 0; feat_index < featNum; feat_index++)
    {
      ...
      if (arc_info.is_root)
	      {
		      const string prefix = prefix_1 + "-BS-";
		      const string &pi =  pInstance->feats[head_id][feat_index];

		      const string &pj = pInstance->feats[child_id][feat_index];

		      const string &pj_L1 = child_id <= 1 ? LEFT_RIGHT_END_POS :pInstance->feats[child_id-1][feat_index]; // pos-j-1

		      const string &pj_R1 = child_id == pInstance->orig_size()-1 ? LEFT_RIGHT_END_POS : pInstance->feats[child_id+1][feat_index]; // pos-j+1
	      ...
	      else
	      {
		      ...
		      const string prefix = "BS-";
		      const string &pi = pInstance->feats[smaller][feat_index];
		      const string &pj = pInstance->feats[larger][feat_index];

		      const string &pi_L1 = (smaller <= 1) ? LEFT_RIGHT_END_POS : pInstance->feats[smaller-1][feat_index]; // pos-i-1
		      const string &pj_R1 = (larger == pInstance->orig_size()-1) ? LEFT_RIGHT_END_POS : pInstance->feats[larger+1][feat_index]; // pos-j+1
		      string pi_R1 = pInstance->feats[smaller+1][feat_index]; // pos-i+1
		      string pj_L1 =  pInstance->feats[larger-1][feat_index]; // pos-j-1
		      if (1 == (larger - smaller)) { // adjacent
			      pi_R1 = BET_NO_POS;
			      pj_L1 = pi_R1;
		      } else if (2 == (larger - smaller)) {
...
int DepPipe::addFeature_unigram_mcdonald06( DepInstance *pInstance, const string &deprel, const int node_id, const bool is_child, const bool is_right_arc, FeatureVec &fv)
{
  ...
  if(options.m_isUseFeats)
  {
    int featNum = pInstance->feats[node_id].size();
    for(int feat_index = 0; feat_index < featNum; feat_index++)
    {
      ostringstream out;
      out.str("");
		  out << feat_index;
      const string prefix = "feats-" + out.str();
      string curfeat = pInstance->feats[node_id][feat_index];
      feat = prefix + "0=" + form + FEAT_SEP + curfeat + str_is_child;
			newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir, fv);	
      feat = prefix + "2=" + curfeat + str_is_child;
			newFeatureCount += add(feat, fv); newFeatureCount += add(feat + dir, fv);
    }
  }
...
// 如果是第一个儿子，那么head_id == child1_id
// 如果是最后一个儿子，那么child1_id == child2_id
...
void DepPipe::fillFeatureVectors(DepInstance *instance, FVS_PROBS* pFvsProbs,	const Parameter &params)
{
  ...
  counter_ = 0;
	t_ = instance->orig_size();

  if(instance->orig_size() > options.m_threadnum)
  {
     t_ = options.m_threadnum;
  }

  int distance = instance->orig_size() / t_;

  if(instance->orig_size() - t_ * distance > 0 )
  {
    distance++;
  }


	t_ = (instance->orig_size() + distance - 1) / distance;
	//cout << "sentence Length:" << instance->orig_size()  << "\tDistance:" << distance << "Total splits:" << t_ << endl;
	
	//threadpool tp;
	//tp = create_threadpool(t_);
	ParseArg* arg[MAX_BUFFER_SIZE];

	for(int i = 0; i < t_; i++)
	{
		//cout << "Total splits:" << t_ << "current:" << "(" << i*distance << "," << i*distance + distance <<  ")" << endl;
		arg[i] = new ParseArg(this, instance, pFvsProbs, &params, i*distance, (i+1)*distance);
		dispatch_threadpool(tp, fillFeatures, (void*)arg[i]);
	}
	//cout << "sentence Length:" << instance->orig_size()  << "\tDistance:" << distance << "Total splits:" << t_ << endl;
	sp_thread_mutex_lock (mut_iter); // for one iteration
	sp_thread_cond_wait (cond_iter_over, mut_iter); 
	sp_thread_mutex_unlock (mut_iter);
...
void DepPipe::fillFeatures(void* _arg)
{
  ParseArg* arg = (ParseArg*)_arg;
	DepPipe *dp = arg->dp;
  dp->fillFeatureVectors(arg->pInst, arg->pFvsProbs, *(arg->pparams),arg->start_pos, arg->end_pos);
}
