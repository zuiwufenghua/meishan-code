...
public:
	...
	virtual void decodeProjective(DepInstance *inst, FVS_PROBS* pFvsProbs, int K, 	
                vector<FeatureVec> &d0, vector<string> &d1, vector<double> &parse_probs)

	{
		MultiArray<int> static_types;
		if(options.m_isLabeled) 
		{
			getTypes(pFvsProbs->nt_probs, inst->size(), static_types);
		}
		
		
		pf.reset(0, inst->size()-1, inst, K);
		pf.viterbi(inst, pFvsProbs, static_types, options.m_isLabeled);
