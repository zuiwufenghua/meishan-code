...
int FeatureDictionary::getFeature(const std::string & name, bool create) {
...
void FeatureDictionary::load(const std::string & fileName, const int cutoff) {
	...
	cerr << "FeatureDictionary : loading from \"" << fileName << "\" " << fixed
			<< setprecision(1) << endl;
...
void FeatureDictionary::save(const std::string & fileName) /*const*/
{
	...
	cerr << "FeatureDictionary : saving to \"" << fileName << "\" " << fixed
			<< setprecision(1) << endl;
	cerr << "FeatureDictionary : " << flush;
...
void FeatureDictionary::map_all(vector<int>& fidx, const list<string>& fstr,
		const bool create) {
...
int FeatureDictionary::map_all(int* const fidx, const list<string>& fstr,
		const bool create) {
	list<string>::const_iterator it = fstr.begin();
	const list<string>::const_iterator it_end = fstr.end();
	int n = 0;
...
void FeatureDictionary::map_all(fvec * const fv, const int offset,
		const std::list<std::string>& fstr, const bool create) {
	assert(offset >= 0);
	assert(fv->idx == 0);
...
void FeatureDictionary::collect_keys(const char ** const keys,
		const int sz) const {
