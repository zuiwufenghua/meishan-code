...
public:
	...
	void map_all(fvec * const fv, const int offset,
			const std::list<std::string>& fstr, const bool create);

	void map_all(std::vector<int>& fidx, const std::list<std::string>& fstr,
			const bool create);

	int map_all(int* const fidx, const std::list<std::string>& fstr,
			const bool create);
