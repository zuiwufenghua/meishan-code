...
public:
  int compareTo(Feature other) {
    if(other.index == -1  || index < other.index)
      return -1;
...
*/	}
	...
	void collectFeatures() {
		...
		int i = 0;
		for (; it != mapFeature.end() ; ++it, ++i) {
      if(it->second > EPS || it->second < -EPS)
      {
			  m_fv.push_back(Feature(it->first, it->second));
      }
	...
	void update(vector<double> &parameters, vector<double> &total, vector<double> &variance,
             double alpha_k, vector<double>& upd, double curUpdSeq) const {
	  update(parameters, total, variance, alpha_k, upd, curUpdSeq, false);
	}
...
//  void addFeatureCount(vector<double> &posCount, vector<double> &negCount, set<int> invalidFeatures, double alpha) const {
//    addFeatureCount(posCount, negCount, invalidFeatures, alpha, false);
//  }

//  void addFeatureCount(vector<double> &posCount, vector<double> &negCount, set<int> invalidFeatures, double alpha, bool negate) const;
