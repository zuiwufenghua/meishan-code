...
unsigned long int getUTF8StringLength(const std::string &s) {
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         cerr << "Warning: " << "in utf.h getUTF8StringLength: std::string '" << s << "' not encoded in unicode utf-8" << endl;  
         return retval;
      }
   }
   if (idx != s.length()) {
...
inline int getCharactersFromUTF8String(const std::string &s, CSentence *sentence) {
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         cerr << "Warning: " << "in utf.h getCharactersFromUTF8String: std::string '" << s << "' not encoded in unicode utf-8" << endl; 
         sentence->push_back("?");
         ++len;
         ++idx;
      }
   ...
   if (idx != s.length()) {
      cerr << "Warning: " << "in utf.h getCharactersFromUTF8String: std::string '" << s << "' not encoded in utf-8" << endl;
      return len;
   }
...
inline std::string getFirstCharFromUTF8String(const std::string &s) {
   ...
   if ((s[0]&0x80)==0) {
      ...
      cerr << "Warning: " << "in utf.h getFirstCharFromUTF8String: std::string '" << s << "' not encoded in unicode utf-8" << endl; 
      return "?";
   }
}
...
inline std::string getLastCharFromUTF8String(const std::string &s) {
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         cerr << "Warning: " << "in utf.h getLastCharFromUTF8String: std::string '" << s << "' not encoded in unicode utf-8" << endl; 
         return "?";
      }
   }
   if (idx != s.length()) {
...
inline string getUTF8CharType(const std::string &s)
{
