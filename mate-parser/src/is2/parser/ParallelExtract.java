		...
		while (true){
			...
			if (labels!=null) {
				...
				for (int l = labels.length - 1; l >= 0; l--) {
					...
					f.clear();
					extractor.first(pos,forms,lemmas, feats, phead, prel, w1, w2, label, f);
			...
			for(int m=sg;m<eg;m++) {
				for(int dir=0;dir<2;dir++) {				
					...
					for (int l = labels.length - 1; l >= 0; l--) {
						...
						f.clear();
						extractor.grandchildren(pos,forms,lemmas, feats, phead, prel, w1, w2, g, label, f);
						lab2[l] = f.getScoreF();
					}
			...
			for(int m=s;m<e;m++) {
				for(int dir=0;dir<2;dir++) {				
					...
					for (int l = labels.length - 1; l >= 0; l--) {
						...
						int label = labels[l];
						f.clear();
						extractor.sibling(pos,forms,lemmas,feats, phead, prel, w1, w2, g, label, f);
						lab2[l] = f.getScoreF();
					}
