...
public class TrainCRF {
	...
	public TrainCRF(String trainingFilename, String devFilename, String testingFilename) throws IOException {
...
//		pipes.add(new FeaturesInWindow("WINDOW=",-1,2, null,true));
		...
		pipes.add(new TokenTextCharPrefix("P3=", 3));
		...
		InstanceList devInstances = new InstanceList(pipe);
		InstanceList testingInstances = new InstanceList(pipe);

		trainingInstances.addThruPipe(new LineGroupIterator(new BufferedReader(new InputStreamReader(
				new FileInputStream(trainingFilename), "UTF8")), Pattern.compile("^\\s*$"), true));
		devInstances.addThruPipe(new LineGroupIterator(new BufferedReader(new InputStreamReader(
				new FileInputStream(devFilename), "UTF8")), Pattern.compile("^\\s*$"), true));
		testingInstances.addThruPipe(new LineGroupIterator(new BufferedReader(new InputStreamReader(
				new FileInputStream(testingFilename), "UTF8")), Pattern.compile("^\\s*$"), true));
		...
		//trainer.addEvaluator(new PerClassAccuracyEvaluator(testingInstances, "dev"));
		trainer.addEvaluator(new TokenAccuracyEvaluator(devInstances, "dev"));
	...
	public static void main (String[] args) throws Exception {
		TrainCRF trainer = new TrainCRF(args[0], args[1], args[2]);
