...
public class CheckForSuffixAnnotation {
	...
	public static void main(String[] args) throws Exception{
		...
		Set<String> validAnnotate = new HashSet<String>();
		validAnnotate.add("x");
		validAnnotate.add("y");
		validAnnotate.add("z");
		...
		while ((sLine = in.readLine()) != null) {
			...
			if(theUnits.length != 2 || !validAnnotate.contains(theUnits[1]) ||
					(suffixMap.containsKey(theUnits[0]) && !suffixMap.get(theUnits[0]).equals(theUnits[1])))
			{
				System.out.println(sLine.trim());
				continue;
			}
			suffixMap.put(theUnits[0], theUnits[1]);
		}
		...
		while ((sLine = in.readLine()) != null) {
			if(sLine.trim().length() < 2) continue;
			//if(sLine.indexOf("(AD#y (AD#b ，) (AD#z (AD#i 今) (AD#i 儿)))") != -1)
			//{
			...
			try
			{
				...
				if(CFGWordStructureNormalize.checkWordStructure(normalizedTree))
				{
					...
					if(thePOS.equals("CD") || thePOS.equals("NT") || thePOS.equals("OD"))continue;
					...
					for(Tree<String> oneSubWordTree : allNonTerminals)
					{
						...
						assert(oneSubWordTree.getChildren().size() == 2);
						//String leftWord = oneSubWordTree.getChild(0).getTerminalStr();
						String rightWord = oneSubWordTree.getChild(1).getTerminalStr();
						...
						assert(validAnnotate.contains(theSubWordHeadLabel));
						if(suffixMap.containsKey(rightWord) && !suffixMap.get(rightWord).equals(theSubWordHeadLabel))
						{
							if(!subwordstructure.containsKey(rightWord))
							{
								subwordstructure.put(rightWord, new HashMap<String, Set<String>>());
							}
							if(!subwordstructure.get(rightWord).containsKey(theSubWordHeadLabel))
							{
								subwordstructure.get(rightWord).put(theSubWordHeadLabel, new HashSet<String>());
							}
							subwordstructure.get(rightWord).get(theSubWordHeadLabel).add(normalizedTree.toString());
						}
					}
				}
			}
