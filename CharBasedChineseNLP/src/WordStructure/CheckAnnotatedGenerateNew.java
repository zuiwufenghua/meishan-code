...
public class CheckAnnotatedGenerateNew {
	...
	//4. output annotated trees
		public static void main(String[] args) throws Exception{
			...
			for(String key: goldTrees.keySet())
			{
				...
				if(pos.equals("CD") || pos.equals("NT") || pos.equals("NR")) continue;
				...
				for(Tree<String> subTree : subTrees)
				{
					...
					if(theWord.length() > 1)
					{
						...
						{
							partiWordAnnotated.put(theWord, new HashMap<String, Integer>());
						}
						
						if(!partiWordAnnotated.get(theWord).containsKey(theTag))
						{
							partiWordAnnotated.get(theWord).put(theTag, 0);
						}
						
						partiWordAnnotated.get(theWord).put(theTag, partiWordAnnotated.get(theWord).get(theTag)+1);
					}
				}
			}
			
			printPartialDict(partiWordAnnotated, args[2]);
