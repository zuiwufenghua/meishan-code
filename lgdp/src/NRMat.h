...
namespace nr {
  ...
  class NRHeap {
    ...
    T add_elem(const T &a, bool &binsert)
    {
      if(ne == maxsize)
      {
        if( compare()(v[0], a) < 0) 
  ...
  protected:
	...
	template <typename T, typename compare>
	NRHeap<T, compare>::NRHeap() : nn(0), ne(0), v(0), maxsize(0) {}
	...
	template <typename T, typename compare>
	NRHeap<T, compare>::NRHeap(const int n) : nn(0), v(0) {
		resize(n);
	}

	template <typename T, typename compare>
	NRHeap<T, compare>::NRHeap(const NRHeap<T, compare> &rhs) : nn(rhs.nn), ne(rhs.ne), maxsize(rhs.maxsize),v(new T[maxsize]) {
